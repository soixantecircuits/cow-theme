...
function wp_admin_bar_valid_search_menu( $wp_admin_bar ) {
	...
	$form .= '<input type="submit" class="adminbar-button" value="' . __('Recherche', 'bel') . '"/>';
	$form .= '</div></form>';
...
function theme_custom_init(){
 $labels = array(
  ...
  'add_new_item' => __('Ajouter un nouveau projet', 'bel'),
  'edit_item' => __('Modifier le projet', 'bel'),
  'new_item' => __('Nouveau projet', 'bel'),
  'all_items' => __('Tous les projets', 'bel'),
...
function generate_custom_lang_qtranslate() {
	...
	foreach(qtrans_getSortedLanguages() as $language) {
		...
		echo '<a href="'.get_site_url().($post != null ? '/?p='.$post->ID.'&':'?').'lang='.$language.'"';
